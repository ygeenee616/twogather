...
export class UploadsController {
  ...
  @ApiBearerAuth('userToken')
  @ApiOperation({
    summary: '공간 이미지 수정 API',
    description: '공간 이미지 수정',
  })
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    schema: {
  ...
  @ApiResponse({
    status: 201,
    description: '변경된 공간 이미지로 등록 완료',
    schema: {
      example: '2개의 spaceImage 파일이 수정 업로드되었습니다.',
    },
  })
  @UseInterceptors(
    FilesInterceptor('images', 4, {
  ...
  async updateSpaceImage(
    ...
    return this.uploadsService.updateSpaceImage(spaceId, files);
  }
  ...
  @ApiBearerAuth('userToken')
  @ApiOperation({
    summary: '룸 이미지 수정 API',
    description: '룸 이미지 수정',
  })
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    schema: {
  ...
  @ApiResponse({
    status: 201,
    description: '변경된 룸 이미지로 등록 완료',
    schema: {
      example: '2개의 roomImage 파일이 수정 업로드되었습니다.',
    },
  })
  @UseInterceptors(
    FilesInterceptor('images', 4, {
  ...
  async updateRoomImage(
    ...
    return this.uploadsService.updateRoomImage(roomId, files);
  }
